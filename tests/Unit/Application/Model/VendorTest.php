...
class VendorTest extends \OxidTestCase
{
    ...
    public function testGetLinkSeoDe()
    {
        ...
        $sQ = 'select oxid from oxvendor where oxvendor.oxshopid = "' . $this->getConfig()->getShopID() . '" order by oxid asc';

        $myDB = oxDb::getDB();
        $sVendorId = $myDB->getOne($sQ);

        $sQ = 'select oxtitle from oxvendor where oxvendor.oxshopid = "' . $this->getConfig()->getShopID() . '" order by oxid asc';
    ...
    public function testGetLinkSeoEng()
    {
        ...
        // fetching first vendor from db
        $sQ = 'select oxid from oxvendor where oxvendor.oxshopid = "' . $myConfig->getShopID() . '" order by oxid asc';
        $sVendorId = $myDB->getOne($sQ);

        $sQ = 'select oxtitle_1 from oxvendor where oxvendor.oxshopid = "' . $myConfig->getShopID() . '" order by oxid asc';
        $sVendorTitle = $myDB->getOne($sQ);
